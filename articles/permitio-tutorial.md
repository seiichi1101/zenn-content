...
emoji: "🙌"
type: "tech"
topics: ["accesscontrol", "permitio", "authorization", "rbac"]
published: true
---
...
ここでのポイントは、HTTP の`method`と`path`をそれぞれ、`action`と`resource`にマッピングしている点です。
...
次に、Permit.io のコンソールからユーザーの Role を`viewer`に変更してみます。
